...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncated_size = random.randint(4, self.max_context_size)
        document = document[:truncated_size]
    
        # Calculate the size of the masked content
        max_masked_size = int(truncated_size * self.masking_percent)
        masked_size = random.randint(0, max_masked_size)
    
        # Calculate the prefix size within valid range
        max_prefix_size = truncated_size - masked_size
        prefix_size = random.randint(0, max_prefix_size)
    
        prefix = document[:prefix_size]
        masked_content = document[prefix_size : prefix_size + masked_size]
        suffix = document[prefix_size + masked_size:]

        # Form the masked string
        masked_string = "{}{}{}{}{}{}".format(
            prefix, 
            self.MASK_CHAR, 
            suffix, 
            self.MASK_CHAR, 
        ...
        # Encode the input and output strings as LongTensors
        x = torch.LongTensor([self.stoi[p] for p in masked_string[:-1]]).view(-1)
        y = torch.LongTensor([self.stoi[p] for p in masked_string[1:]]).view(-1)
