...
class DownProjectBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.ln_2 = nn.LayerNorm(config.bottleneck_dim)
    ...
    def forward(self, x_input):
        ...
        ### START CODE HERE
        ### END CODE HERE
        C_prime = self.ln_1(self.C)
        a = self.attn(self.ln_2(x_input), C_prime)
        return self.mlp(a + x_input)
...
class UpProjectBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        ### START CODE HERE
        ### END CODE HERE
        self.ln_1 = nn.LayerNorm(config.bottleneck_dim)
        self.ln_2 = nn.LayerNorm(config.n_embd)
        self.attn = CausalCrossAttention(config)
        self.mlp = nn.Sequential(
            nn.Linear(config.bottleneck_dim, 4 * config.bottleneck_dim),
            nn.GELU(),
            nn.Linear(4 * config.bottleneck_dim, config.bottleneck_dim),
        )
    ...
    def forward(self, y, x_input):
        ...
        ### START CODE HERE
        ### END CODE HERE
        x_input = self.ln_1(x_input)
        a = self.attn(self.ln_2(y), x_input)
        return self.mlp(a + y)
